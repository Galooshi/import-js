        ...
        resp.files.forEach((file: Object) => {
          ...
          if (file.exists) {
            if (normalizedPath === './package.json') {
              added.push(...findPackageDependencies(
                this.workingDirectory, true).map((dep: string): Object => ({
                  path: normalizePath(requireResolve(path.join(
                    this.workingDirectory, 'node_modules', dep), this.workingDirectory)),
                  mtime: +file.mtime_ms,
                  alias: dep,
                })));
            } else {
