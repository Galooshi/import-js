...
class Configuration {
  constructor(pathToCurrentFile) {
    ...
    if (userConfig) {
      this.configs.concat(flattenDeep([userConfig]).reverse);
    }
  ...
  get(key, opts) {
    return this.configs.find((config) => {
      ...
      return minimatch(this.pathToCurrentFile,
                       this._normalizePath(appliesTo))  &&
             minimatch(this._normalizePath(opts && opts.fromFile),
