...
class Configuration {
  initialize(pathToCurrentFile) {
    ...
    if (userConfig) {
      this.configs.concat(flatten([user_config]).reverse);
    }
  ...
  get(key, { fromFile }) {
    return this.configs.find((config) => {
      ...
      return minimatch(this._normalizePath(appliesTo), this.pathToCurrentFile) &&
        minimatch(this._normalizePath(appliesFrom), this._normalizePath(fromFile));
  ...
  resolveAlias(variableName, pathToCurrentFile) {
    let path = get('aliases')[variableName];
    if (!path) {
      return;
    }
  ...
  resolveNamedExports(variableName) {
    ...
    Object.keys(allNamedExports).forEach((importPath) => {
      ...
      const jsModule = new JsModule({ importPath: importPath });
  ...
  _normalizePath(path) {
    ...
    path = path.replace(RegExp(`^${regExpEscape(process.cwd)}`), '.')
    if (!path.startsWith('.')) {
      path = './' + path;
    }
    return path;
