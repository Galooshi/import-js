...
function convertToImportStatement(node, source) {
  ...
  if (node.type === 'VariableDeclaration') {
    ...
    const defaultImport =
      declaration.id.type === 'Identifier' ? declaration.id.name : undefined;

    const namedImports =
      declaration.id.type === 'ObjectPattern'
        ? declaration.id.properties.map((p) => ({
...
export default function findCurrentImports(config, currentFileContent, ast) {
  const result = {
  ...
  ast.program.body.forEach((node, index) => {
    ...
    const importStatement = convertToImportStatement(
      node,
