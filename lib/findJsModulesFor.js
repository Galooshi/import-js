...
function findImportsFromModuleFinder(
  ...
  return new Promise((resolve: Function, reject: Function) => {
    let exports;
    try {
      exports = finder.find(variableName);
    } catch (e) {
      reject(e);
      return;
    }
    const modules = Object.keys(exports).map((filePath: string): JsModule =>
      JsModule.construct({
        hasNamedExports: exports[filePath] !== 'default',
        relativeFilePath: filePath,
        stripFileExtensions:
          config.get('stripFileExtensions', { pathToImportedModule: filePath }),
        makeRelativeTo:
          config.get('useRelativePaths', { pathToImportedModule: filePath }) &&
          pathToCurrentFile,
        variableName,
        workingDirectory: config.workingDirectory,
      })
