...
program
  ...
  .option(...sharedOptions.overwrite)
  .action((word: string, pathToFile: string, options: Object) => {
    const executor = (importer: Importer): Promise<Array<JsModule>> => importer.search(word).then(({
      modules,
    }: {
...
program
  ...
  .action((imports: string, pathToFile: string, options: Object) => {
    const executor = (importer: Importer): Promise<Object> => importer.addImports(JSON.parse(imports));
    runCommand(executor, pathToFile, options);
  });
