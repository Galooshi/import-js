...
function runCommand(
  ...
  return initializeModuleFinder(workingDirectory)
    .then((): Promise<*> => getLines(pathToFile).then((
      lines: Array<string>,
    ): Promise<*> => {
...
program.command('fix <pathToFile>').option(...sharedOptions.overwrite).action((
  pathToFile: string,
  options: Object,
) => {
  const executor = (importer: Importer): Promise<Object> =>
...
program.command('goto <word> <pathToFile>').action((
  ...
  initializeModuleFinder(workingDirectory)
    .then((): Promise<*> => getLines(pathToFile).then((
      ...
      new Importer(lines, pathToFile, workingDirectory).goto(word).then((
        result: Object,
      ) => {
        stdoutWrite(JSON.stringify(result));
        process.exit(0);
