...
function findESNamedExports(node, { absolutePathToFile }) {
  if (node.type === 'ExportAllDeclaration') {
    ...
    return resolveNestedNamedExports(node, absolutePathToFile, false);
  }
...
function resolveNestedNamedExports(node, absolutePathToFile, useUnsafeRequireParsing) {
  if (node.type === 'ConditionalExpression') {
    // Potential ternary-style export - we pick the first one
    // module.exports = foo ? require('a') : require('b');
    return resolveNestedNamedExports(node.consequent, absolutePathToFile, useUnsafeRequireParsing);
  }
  if (
    ...
    // eslint-disable-next-line no-use-before-define
    const { named } = findExports(requiredFileContent, pathToRequiredFile, useUnsafeRequireParsing);
    return named;
...
function getExportsByRequire(path) {
  try {
    // eslint-disable-next-line global-require, import/no-dynamic-require
    const moduleExports = require(path);
    return Object.keys(moduleExports.default || moduleExports);
  } catch (e) {
    return undefined;
  }
}
...
export default function findExports(data, absolutePathToFile, useUnsafeRequireParsing) {
  ...
  let hasDefault =
    hasDefaultExport(rootNodes) || aliasesForExports.size > 1 || named.indexOf('default') !== -1;
  if (!hasDefault) {
  ...
  if (!named.length) {
    let rawExports = findRawNamedExports(data);
    winston.debug(`useUnsafeRequireParsing: ${useUnsafeRequireParsing}`);
    if (!rawExports && useUnsafeRequireParsing) {
      rawExports = getExportsByRequire(absolutePathToFile);
    }
    named.push(...(rawExports || []));
  }
