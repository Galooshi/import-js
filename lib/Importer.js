...
class Importer {
  ...
  replaceImports(oldImportsRange, newImports) {
    ...
    if (oldImportsRange.length + importStrings.length > 0 &&
        this.editor.readLine(lastNumInRange + 1) !== '') {
      this.editor.appendLine(lastNumInRange, '');
    }
    ...
    oldImportsRange.forEach(() => this.editor.deleteLine(oldImportsRange[0]));
  ...
  findCurrentImports() {
    ...
    if (xRegExp.test(skipped, xRegExp('^(\\s*\\n)+$', 's'))) {
