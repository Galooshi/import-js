...
import getEslintWithPathCorrected from './getEslintWithPathCorrected';
...
export default class Importer {
  ...
  fixImports(): Promise<Object> {
    const eslint = getEslintWithPathCorrected(this.workingDirectory);
    ...
    this.config.get('moduleSideEffectImports').forEach((path: string) => {
      newImports.push(new ImportStatement({
        namedImports: [],
        defaultImport: '',
        hasSideEffects: true,
  ...
  fixImportsMessage(removedVariables: Set<string>, addedVariables: Set<string>): ?string {
    const messageParts = [];

    const firstAdded = addedVariables.values().next().value;
    const firstRemoved = removedVariables.values().next().value;

    if (addedVariables.size === 1 && firstAdded) {
      messageParts.push(`Imported \`${firstAdded}\`.`);
    } else if (addedVariables.size) {
      messageParts.push(`Added ${addedVariables.size} imports.`);
    }

    if (removedVariables.size === 1 && firstRemoved) {
