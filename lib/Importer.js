...
class Importer {
  ...
  // @param {Range} oldImportsRange
  // @param {ImportJS::ImportStatements} newImports
  ...
  findJsModulesFor(variableName) {
    ...
    matchedModules = sortBy(matchedModules,
      (module) => module.importPath.length); // TODO: do we need this sortBy?
    matchedModules = uniqBy(matchedModules, m => `${m.lookupPath}/${m.importPath}`);
