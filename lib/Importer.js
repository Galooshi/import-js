...
class Importer {
  ...
  anyNumbersWithinRange(numbers, range) {
    return numbers.some(n => range.includes(n));
  }
  ...
  replaceImports(oldImportsRange, newImports) {
    ...
    if (oldImportsRange.size() + importStrings.length > 0 &&
        this.editor.readLine(oldImportsRange.end) !== '') {
      this.editor.appendLine(oldImportsRange.end - 1, '');
    }
    ...
    oldImportsRange.forEach(() => this.editor.deleteLine(oldImportsRange.start));

    if (importStrings.length === 0 &&
        this.editor.readLine(oldImportsRange.start) === '') {
      ...
      // state.
      this.editor.deleteLine(oldImportsRange.start);
      return;
    }
    ...
    importStrings.reverse().forEach((importString) => {
      ...
      if (importString.indexOf('\n') !== -1) {
        importString.split('\n').reverse().forEach((line) => {
          this.editor.appendLine(oldImportsRange.start - 1, line);
        });
      } else {
        this.editor.appendLine(oldImportsRange.start - 1, importString);
      }
    });
  }
  ...
  findCurrentImports() {
    ...
    return {
      imports,
      range: new Range(importsStartAtLineNumber, importsEndAtLineNumber),
    };
  }
  ...
  findJsModulesFor(variableName) {
    ...
    return matchedModules.sort((a, b) => a.displayName().localCompare(b.displayName()));
  }
