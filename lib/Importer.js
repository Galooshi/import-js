...
class Importer {
  ...
  fixImports() {
    ...
    const eslintResult = cli.executeOnText(
      this.editor.currentFileContent(), this.editor.pathToCurrentFile()
    ).results[0];

    const unusedVariables = {};
    const undefinedVariables = new Set();

    eslintResult.messages.forEach((message) => {
      if (message.ruleId === 'no-unused-vars') {
        const match = message.message.match(
          /\'(.*)\' is defined but never used/);
        const variableName = match[1];
        unusedVariables[variableName] = unusedVariables[variableName] || new Set();
        unusedVariables[variableName].add(message.line);
      } else if (message.ruleId === 'no-undef') {
        const match = message.message.match(
          /\'(.*)\' is not defined/);
    ...
    // TODO: Filter out unused variables that do not appear within the imports
    // block.
    ...
    newImports.deleteVariables(Object.keys(unusedVariables));
  ...
  replaceImports(oldImportsRange, newImports) {
    ...
    if (oldImportsRange.length + importStrings.length > 0 &&
