...
class Importer {
  constructor(editor) {
    this.editor = editor;
    this.config = new Configuration(editor.pathToCurrentFile());
  }
  ...
  goto(variableName) {
    ...
    return jsModule.openFilePath(this.editor.pathToCurrentFile());
  }
  ...
  resolveOneJsModule(jsModules, variableName) {
    ...
    const selectedIndex = this.editor.resolveImport(
      variableName,
