...
const multilineRegExp = require('lib/multilineRegExp');
const regExpEscape = require('lib/regExpEscape');

const uniq = require('underscore').uniq;
...
class Importer {
  ...
  goto() {
    this.reloadConfig();
    variableName = this.editor.currentWord();
    const jsModules = this.findJsModulesFor(variableName);

    const jsModule = thist.resolveModuleUsingCurrentImports(
    ...
    if (!jsModule) {
      ...
      return this.message("Could not resolve a module for `#{variableName}`")
    }
    ...
    oldImports.imports.forEach((imp) => {
      imp.variables.forEach((variable) => {
        const jsModule = this.resolveModuleUsingCurrentImports(
          findJsModulesFor(variable), variable);
        if (!jsModule) {
          next;
        }
  ...
  replaceImports(oldImportsRange, newImports) {
    ...
    importStrings.reverse().forEach((importString) => {
      ...
      if (importString.indexOf("\n") !== -1) {
        ...
        this.editor.append_line(old_imports_range.first - 1, import_string)
      }
    });
  }
  ...
  findJsModulesFor(variableName) {
    ...
    this.config.get('lookup_paths').forEach((lookup_path) => {
      if (lookupPath === '') {
        // If lookupPath is an empty string, the `find` command will not work
        // as desired so we bail early.
        throw new Error(`lookup path cannot be empty (${lookup_path.inspect})`);
      }
    ...
    const depRegex = multilineRegExp([
      '^(?:',
      ignorePrefixes.join('|'),
      ')?',
      formattedVarName,
  ...
  resolveModuleUsingCurrentImports(jsModules, variableName) {
    ...
    // variable.
    const matchingImportStatement = findCurrentImports.imports.find((ist) => {
    ...
    if (matchingImportStatement) {
      ...
      jsModules.forEach((jsModule) => {
        if (matchingImportStatement.path === jsModule.importPath)  {
          return jsModule;
        }
      });
    }
