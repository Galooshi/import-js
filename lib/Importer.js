...
function findFilePathFromImports(
  ...
  const importStatement = imports.find(
    (is: ImportStatement): boolean => is.hasVariable(variableName),
  );
...
export default class Importer {
  ...
  search(variableName: string): Promise<Object> {
    return findJsModulesFor(
      ...
      variableName,
      { search: true },
    ).then((modules: Array<JsModule>): Object => ({
      modules,
  ...
  addImports(imports: Object): Promise<Object> {
    return new Promise((resolve: Function, reject: Function) => {
      ...
      const promises = variables.map(
        (variableName: string): Promise<void> => findJsModulesFor(this.config, variableName)
          .then((jsModules: Array<JsModule>) => {
            ...
            const foundModule = jsModules.find(
              (jsModule: JsModule): boolean => jsModule.importPath === importPath
  ...
  rewriteImports(): Object {
    ...
    return new Promise((resolve: Function, reject: Function) => {
      ...
      const promises = variables.map((variable: string): Promise<Array<JsModule>> => findJsModulesFor(this.config, variable));
  ...
  resolveOneJsModule(
    ...
    this.unresolvedImports[variableName] = jsModules.map((jsModule: JsModule): Object => ({
    ...
    })).sort((a: Object, b: Object): number => countSeparators(a.data.importPath) - countSeparators(b.data.importPath));
  ...
  resolveModuleUsingCurrentImports(
    ...
    // variable.
    const matchingImportStatement = this.findCurrentImports().imports.find((ist: ImportStatement): boolean => ist.hasVariable(variableName));
    ...
    if (jsModules.length > 0) {
      ...
      return jsModules.find((jsModule: JsModule): boolean => matchingPath === jsModule.toImportStatement(this.config).path);
    }
