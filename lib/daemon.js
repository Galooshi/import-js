...
export default function daemon(parentPid, pathToLogFile) {
  ...
  if (parentPid) {
    ...
    setInterval(() => {
      winston.debug(
        `Making sure that the parent process (PID=${parentPid}) is still running.`,
      );
      try {
  ...
  rlInterface.on('line', (jsonPayload) => {
        ...
        .then((result) => {
          if (isFreshInstall) {
            result.messages.unshift(
              `ImportJS is initializing for ${workingDirectory}. Results will be more accurate in a few moments.`,
            );
          }
