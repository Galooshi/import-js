...
commander.command('start')
  ...
  .action(({ parentPid }) => {
    ...
    loglevelMessagePrefix(loglevel, {
      ...
      staticPrefixes: [`PID:${parentPid}`],
    });
    originalConsoleLog(
      `ImportJS (v${version()}) DAEMON active. Logs will go to: ${pathToLogFile}`);

    ModuleFinder.getForWorkingDirectory(
      process.cwd(), config.get('excludes')).initialize()
      .then(() => {
        loglevel.info(`ModuleFinder is enabled for ${process.cwd()}`);
      })
      .catch((err: Object) => {
        loglevel.info(
          `ModuleFinder active, but can't enable watcher. Reason:\n${err.stack}`);
      });
