...
export default function daemon() {
  ...
  originalConsoleLog(`DAEMON active. Logs will go to: ${pathToLogFile}`);
  ...
  if (commander.parentPid) {
    ...
    // once in a while.
    setInterval(() => {
      loglevel.debug(`Making sure that the parent process (PID=${commander.parentPid}) is still running.`);
      try {
