...
export default class JsModule {
  ...
  resolvedFilePath(pathToCurrentFile: string): string {
    if (this.filePath) {
  ...
  toImportStatement(
    ...
    const fromFile = this.resolvedFilePath(config.pathToCurrentFile);

    return new ImportStatement({
      declarationKeyword:
        config.get('declarationKeyword', { fromFile }),
      defaultImport,
      importFunction: config.get('importFunction', { fromFile }),
      namedImports,
