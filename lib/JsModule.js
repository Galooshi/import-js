...
function resolveImportPathAndMain(filePath, stripFileExtensions) {
  if (filePath.endsWith('/package.json')) {
    ...
    const fileContents = File.read(filePath);
    if (!fileContents.trim().length) {
      return [nil, nil];
    }

    let mainFile = JSON.parse(fileContents).main;
    ...
    if (fs.lstatSync(`${match.package}/${mainFile}`).isDirectory()) {
      // The main in package.json refers to a directory, so we want to
      // resolve it to an index file.
      const indexFile = findIndex(`${match.package}/${mainFile}`);
      if (indexFile) {
        mainFile += `/${indexFile}`;
      }
    }
  ...
  const extensions = stripFileExtensions.map((ext) => regExpEscape(ext));
...
function findIndex(directory) {
  ['index.js', 'index.jsx'].forEach((indexFile) => {
    if (fs.existsSync(`${directory}/${indexFile}`)) {
      return indexFile;
    }
  });
}
...
class JSModule {
  ...
  static construct({
    ...
    const importPath = importPath.replace(
      RegExp(`^${regExpEscape(jsModule.lookupPath)}`), '');
  ...
  makeRelativeTo(makeRelativeTo) {
    ...
    makeRelativeTo = makeRelativeTo.replace(`${process.cwd()}/`, '');
    ...
    makeRelativeTo = makeRelativeTo.replace(
      RegExp(`^${regExpEscape(this.lookupPath)}/`), '');

    let path = path.relative(importPath, makeRelativeTo);
  ...
  toImportStatement(variableName, config) {
    let namedImports, defaultImport;
    ...
    return new ImportStatement({
      declarationKeyword:
        config.get('declaration_keyword', { fromFile: filePath }),
      defaultImport: defaultImport,
      importFunction: config.get('import_function', { fromFile: filePath }),
