...
describe('Importer', () => {
  ...
  beforeEach(() => {
    ...
    setup = () => {
      ...
      FileUtils.__setFile(
        path.join(process.cwd(), 'package.json'), { dependencies });
      ...
      packageDependencies.forEach((dep) => {
        requireResolve.__addResolvedPath(dep,
          `node_modules/${dep}/${dep}-main.jsx`);
      });
    ...
    beforeEach(() => {
      subject = () => {
        ...
        new Importer(text.split('\n'), pathToCurrentFile)
          .import(word).then((promiseResult) => {
...
foo
      ...
      it('adds the import below a one-line comment at the top of the file', () => {
        text = `
// One-line comment
...
foo
      ...
      it('adds the import below multiple one-line comments at the top of the file', () => {
        text = `
// One-line comment
...
foo
      ...
      it('does not preserve an empty line at the top of the file', () => {
        text = `
foo`;
        expect(subject()).toEqual(`
import foo from './bar/foo';
...
foo
      ...
      it('adds the import below multi-line comments at the top of the file', () => {
        text = `
/* Multi-line comment */
...
foo
      ...
      it('adds the import below both comment styles at the top of the file', () => {
        text = `
// One-line comment
/* Multi-line comment */
...
foo
      ...
      it("adds the import below both styles of comments and 'use strict'", () => {
        text = `
'use strict';
// One-line comment
/* Multi-line comment */
...
$
      ...
      describe('with `namedExports` object', () => {
        ...
        describe('when the named export is a node_module', () => {
          beforeEach(() => {
            requireResolve.__addResolvedPath('lib/utils',
...
foo
        ...
        describe('when the named export is a regular file', () => {
          beforeEach(() => {
            requireResolve.__addResolvedPath('lib/utils', 'lib/utils.js');
          });

          it('resolves that import and uses a relative path', () => {
            expect(subject()).toEqual(`
import { foo } from './lib/utils';
...
foo
      ...
      describe('with local configuration defined in the main config file', () => {
        ...
        it('uses local config when the pattern matches the file being edited', () => {
          expect(subject()).toEqual(`
...
goo
        ...
        it('falls back to default when the pattern does not match file being edited', () => {
...
goo
        ...
        describe('with an appliesFrom pattern and alias', () => {
          beforeEach(() => {
            word = 'foobar';
            text = 'foobar';
          ...
          it('uses default config when alias path does not match appliesFrom', () => {
            expect(subject()).toEqual(`
import foobar from 'jar/jar/binks';
...
foobar
          ...
          it('uses local config when alias path matches appliesFrom', () => {
            configuration.appliesFrom = 'jar/**';
            expect(subject()).toEqual(`
const foobar = require('jar/jar/binks');
  ...
  describe('#addImports', () => {
    ...
    beforeEach(() => {
      subject = () => {
        ...
        let done = false;
        new Importer(text.split('\n'), pathToCurrentFile)
          .addImports(resolvedImports).then((promiseResult) => {
            result = promiseResult;
            done = true;
          })
          .catch((promiseError) => {
...
foo();
      ...
      describe('when that import already exists as a side-effect import', () => {
        beforeEach(() => {
          text = `
import './bar/foo';
...
foo();
    ...
    describe('when there are multiple `moduleSideEffectImports`', () => {
      beforeEach(() => {
        existingFiles = [];
        text = 'foo();';
        configuration.moduleSideEffectImports = () => ['surstromming', 'pitepalt'];
      });
  ...
  describe('#rewriteImports', () => {
    ...
    beforeEach(() => {
      ...
      subject = () => {
        ...
        let done = false;
        new Importer(text.split('\n'), pathToCurrentFile)
          .rewriteImports().then((promiseResult) => {
            result = promiseResult;
            done = true;
          })
          .catch((promiseError) => {
