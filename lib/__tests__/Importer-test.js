...
describe('Importer', () => {
  ...
  beforeEach(() => {
    ...
    setup = () => {
      ...
      findMatchingFiles.__setExistingFiles(existingFiles.map((f) => `app/${f}`));
    };
  });
        ...
        it('records the alternatives to choose from', () => {
          ...
          expect(result.unresolvedImports).toEqual({
            foo: [
              {
                displayName: 'bar/foo',
                filePath: 'app/bar/foo.jsx',
                importPath: 'bar/foo',
              },
              {
                displayName: 'zoo/foo',
                filePath: 'app/zoo/foo.js',
                importPath: 'zoo/foo',
              },
              {
                displayName: 'zoo/goo/Foo (main: index.js)',
                filePath: 'app/zoo/goo/Foo/index.js',
...
foo
      ...
      describe('with `useRelativePaths=true`', () => {
        ...
        describe('when the current file is in the same lookupPath', () => {
          beforeEach(() => {
            pathToCurrentFile = 'app/bar/current.js';
          });
...
goo
            ...
            describe('when using `.` as a lookupPath', () => {
              beforeEach(() => {
                configuration.lookupPaths = ['.'];
              });

              it('uses local config', () => {
                // TODO: Is there a bug here?
                expect(subject()).toEqual(`
var goo = quack('../app/bar/goo.jsx');
...
export default function foo() {
  ...
  describe('#rewriteImports', () => {
    ...
    beforeEach(() => {
      ...
      pathToCurrentFile = 'app/app/bilbo/frodo.js';
...
bar
  ...
  describe('#goto', () => {
    ...
    describe('with a variable name that will resolve', () => {
      ...
      it('opens the file', () => {
        expect(subject()).toEqual('app/bar/foo.jsx');
      });
