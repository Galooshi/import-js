  ...
  beforeEach(() => {
    ...
    setup = async () => {
      ...
      const dependencies = packageDependencies.reduce(
        (depsObj, dependency) => ({ ...depsObj, [dependency]: '1.0.0' }),
        {},
      );
      ...
      await moduleFinder.handleFilesAdded(
        existingFiles.map((f) => ({
...
foo
      ...
      it('adds the import below a one-line comment at the top of the file', async () => {
        text = `
// One-line comment
...
foo
      ...
      it('adds the import below multiple one-line comments at the top of the file', async () => {
        text = `
// One-line comment
...
foo
      ...
      it('adds the import below multi-line comments at the top of the file', async () => {
        text = `
/* Multi-line comment */
...
foo
      ...
      it('adds the import below both comment styles at the top of the file', async () => {
        text = `
// One-line comment
/* Multi-line comment */
...
foo
      ...
      it("adds the import below both styles of comments and 'use strict'", async () => {
        text = `
'use strict';
// One-line comment
/* Multi-line comment */
...
Meteor
      ...
      it('adds a require to the top of the buffer in a node environment', async () => {
        word = 'Readline';
        text = 'Readline';
...
var import_foo = { from: b }
        ...
        it('adds the import, sorts the entire list and keeps the line-break', async () => {
          expect(await subject()).toEqual(
            `
import foo from './bar/foo';
import tsar from './foo/bar';
...
foo
            ...
            it('changes the `var` to declarationKeyword and removes space', async () => {
              expect(await subject()).toEqual(
                `
const foo = require('./bar/foo');
...
foo
            ...
            it('changes `var` to declarationKeyword and doubles to singles', async () => {
              expect(await subject()).toEqual(
                `
import foo from './bar/foo';
...
foo
      ...
      describe('when the resolved import is named', () => {
        beforeEach(() => {
          readFile.mockImplementation(() =>
            Promise.resolve('const foo = "foo"; export { foo };'),
          );
        });
...
foo
      ...
      it('displays a message', async () => {
        await subject();
        expect(result.messages).toEqual([
          "Imported foo from './bar/foo'. Removed `bar`.",
        ]);
      });
    });
...
export default function foo() {
  ...
  describe('#rewriteImports', () => {
    ...
    beforeEach(() => {
      ...
      subject = async () => {
        await setup();
        const result = await new Importer(
          text.split('\n'),
          pathToCurrentFile,
          process.cwd(),
        ).rewriteImports();
...
foo
    ...
    describe('with a variable name that will resolve to a package dependency', () => {
      ...
      it('opens the `main` file', async () => {
        expect(await subject()).toEqual(
          path.join(process.cwd(), 'node_modules/foo/foo-main.jsx'),
        );
      });
    });
    ...
    describe('with a variable name matching an alias', () => {
      ...
      describe('to an absolute resource', () => {
        ...
        it('opens the alias main file', async () => {
          expect(await subject()).toEqual(
            path.join(process.cwd(), 'node_modules/stylez/stylez-main.jsx'),
          );
        });
      });
    });
