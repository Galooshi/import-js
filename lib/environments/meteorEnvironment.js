...
function meteorPackageDependencies({ config }) {
  ...
  const packages = (
    ...
    .map((pkg) => `meteor/${pkg.trimLeft()}`)
    // eliminate those packages that are considered to be core
    .filter((pkg) => !coreModulesSet.has(pkg));
  return packages;
}
...
function meteorPackageVersions(projectRootDir) {
  ...
  pkgVersionPairs.forEach((pkgVersionPair) => {
...
function extractExportsFromMeteorPackage(projectRootDir, pkg, pkgVersion) {
  ...
  isopackVer.builds.forEach((build) => {
    ...
    buildIsopack.declaredExports.forEach((declaredExport) => {
...
function meteorPackageNamedExports({ config }) {
  ...
  meteorPackageDependencies({ config }).forEach((meteorPkg) => {
...
export default {
  ...
  moduleSideEffectImports({ pathToCurrentFile, config }) {
    let basePath = '';
    ...
    ['.html', '.css'].forEach((ext) => {
      ...
      if (fs.existsSync(path.join(config.workingDirectory, moduleSpecifier))) {
        if (config.get('useRelativePaths')) {
          moduleSpecifiers.push(`./${path.basename(moduleSpecifier)}`);
        } else {
