...
function extractExportsFromMeteorPackage(
  ...
  isopackVer.builds.forEach((build: Object) => {
    ...
    buildIsopack.declaredExports.forEach((declaredExport: Object) => {
      ...
      if (declaredExports.indexOf(declaredExport.name) !== -1) {
        return;
      }

      // This declaredExport is not a "testOnly" export and is not already
      // amongst those we've gathered, so we want to push it into our
      // collection.
