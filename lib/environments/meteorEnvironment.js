...
function extractExportsFromMeteorPackage(
  ...
  // TODO: When https://github.com/facebook/flow/pull/1668 is fixed, this can
  // be changed back. The [...declaredExports] statement at the end fails now.
  const declaredExports: any = new Set();
  ...
  for (const build of isopackVer.builds) {
    ...
    if (!buildIsopack) {
      // This build is missing or corrupted. Try the next one.
      continue;
    }
    if (buildIsopack.declaredExports) {
      for (const declaredExport of buildIsopack.declaredExports) {
        // If this declaredExport is not a "testOnly" export and is not already
        // amongst those we've gathered, push it into our collection.
    ...
    if (buildIsopack.resources) {
      // Check whether the isoPack constains a mainModule specification
      const mainModule = buildIsopack.resources.find((resource: Object): boolean =>
        resource.fileOptions && resource.fileOptions.mainModule);
      if (mainModule) {
        // Find the mainModule, parse it, and add any identified exports to the
        // list of all declared package exports.
        const mainModulePath = path.join(isopackRoot, mainModule.file);
        const xface = extractInterface(mainModulePath);
        if (xface) {
          xface.forEach((namedExport: string) => {
            // TODO: this is currently a gross oversimplification of what needs
            // to happen here. importName tokens (stars) that have not been
            // aliased signal a need to go to another file and include its
            // namespace in this one directly. That can't be done in the generic
            // extractInterface routine because it doesn't know how to find the
            // file. That process differs from one environment to another. So
            // this level needs to be able to follow that recursion calling
            // extractInterface repetitively to eventually gather the whole
            // interface. That means that extractInterface needs to return the
            // moduleName from the "import * from moduleName;" statement. Think
            // about having it return an object like the one's described in the
            // ecmascript specification to define module interfaces though it
            // could just return '*moduleName' since no legal identifier starts
            // with *.
            if (!namedExport.startsWith('*')) {
              declaredExports.add(namedExport);
            }
          });
        }
      }
...
export default {
  ...
  // exclude all directories starting with '.' and all private and public directories at any
  // level of the directory tree.
  excludes: ['**/@(.[^.]*|private|public)/**'],
    ...
    ['.html', '.css'].forEach((ext: string) => {
      ...
      if (fs.existsSync(path.join(config.workingDirectory, moduleSpecifier))) {
        if (config.get('useRelativePaths')) {
          ...
          // Strip the leading '.' off of the moduleSpecifier to turn it into a Meteor compliant
          // absolute path.
