...
function checkWorkingDirectory(workingDirectory) {
  return new Promise((resolve, reject) => {
    if (
      REQUIRED_FILES.some((file) => fs.existsSync(path.join(workingDirectory, file)))
    ) {
...
export default function initializeModuleFinder(workingDirectory = process.cwd()) {
  ...
  return new Promise((resolve, reject) => {
    checkWorkingDirectory(workingDirectory)
      ...
      .then(({ isFreshInstall }) => moduleFinder.startWatcher().then(() => {
