...
export default class ImportStatement {
  ...
  static parse(string: string): ?ImportStatement {
    ...
    // Assume that an import with no assignment has side-effects.
    const hasSideEffects = !match.assignment;
  ...
  toImportStrings(maxLineLength: number, tab: string): Array<string> {
    ...
    if (this.hasSideEffects) {
      ...
      return [this._sideEffectImportString()];
    }
  ...
  _sideEffectImportString(): string {
    if (this.declarationKeyword === 'import') {
