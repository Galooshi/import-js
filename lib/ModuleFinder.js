...
function expandFiles(files, workingDirectory) {
  ...
  files.forEach((file) => {
    ...
    findPackageDependencies(workingDirectory, true).forEach((dep) => {
      let resolvedPath;
      try {
        resolvedPath = forwardSlashes(path.relative(
          workingDirectory,
      ...
      promises.push(lastUpdate.failSafe(resolvedPath, workingDirectory)
        .then(({ mtime }) =>
          Promise.resolve({
            path: resolvedPath,
            mtime,
...
function defaultExportNames(pathToFile) {
  ...
  if (/e?s$/.test(dirName)) {
    ...
    result.push({
      name: `${dirName.replace(/e?s$/, '')}-${fileName}`,
      additional: true,
    });
  }
  return result;
...
export default class ModuleFinder {
  ...
  static getForWorkingDirectory(
    workingDirectory,
    { excludes, useUnsafeRequireParsing, ignorePackagePrefixes },
  ) {
  ...
  constructor(workingDirectory, { excludes, useUnsafeRequireParsing, ignorePackagePrefixes }) {
    this.excludes = excludes;
    this.useUnsafeRequireParsing = useUnsafeRequireParsing;
  ...
  processQueue(done) {
    ...
    readFile(fullPath)
      .then((data) => {
        ...
        try {
          exports = findExports(data, fullPath, this.useUnsafeRequireParsing);
        } catch (e) {
        ...
        this.storage
          ...
          .catch((error) => {
            winston.error(`Failed to update ${pathToFile} in storage. Reason: ${error.message}`);
            this.processQueue(done);
          });
      })
      .catch((err) => {
